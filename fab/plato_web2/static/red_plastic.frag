...
vec3 getNormal() {
  ...
  // vec3 rand_vec = 0.1 * normalize(vec3(rand(vec2(dPositiondx.x, dPositiondy.x)),
  //                                       rand(vec2(dPositiondx.y, dPositiondy.y)),
  //                                       rand(vec2(dPositiondx.z, dPositiondy.z))));
  // return normalize(normalize(cross(dPositiondx, dPositiondy)) + rand_vec);
  return normalize(cross(dPositiondx, dPositiondy));
}

// THIS CODE COPIED FROM gold.frag
...
vec4 plastic_color(vec4 color, vec4 dark, vec4 light) {
  ...
  // Add in another spotlight
  vec3 spotLight2 = vec3(0, 5, 0);
  vec4 spot_color2 = vec4(0.93, 0.74, 1.0, 1.0);
  spot = dot(normalize(spotLight2 - vPosition), normal);
  spot = max(0.0, spot);
  color = mix(color, spot_color2, 0.2 * spot);

  halfVector = normalize(normalize(cameraPosition - vPosition) + normalize(spotLight2 - vPosition));
  specular = dot(normal, halfVector);
  specular = max(0.0, specular);
  specular = pow(specular, 50.0);
  color = mix(color, spot_color2 * 1.1, 0.1 * specular);
  //
...
void main() {
  ...
  //                         vec3(1.0/3.14, 0.1/3.14, 0.3/3.14),
  ...
  vec4 plast = plastic_color(
    vec4(1.0, 0, 0, 1.0),
    vec4(0, 0, 0, 1.0),
    vec4(1.0, 1.0, 1.0, 1.0));


//  vec4 composite =  brdf_vec4 + plast * 0.5; //base; //mix(brdf_vec4, red_plast, 0.8);

 // composite = clamp(composite, plast * 0.5, vec4(1.0, 1.0, 1.0, 1.0));

  gl_FragColor = plast; //composite;
}
